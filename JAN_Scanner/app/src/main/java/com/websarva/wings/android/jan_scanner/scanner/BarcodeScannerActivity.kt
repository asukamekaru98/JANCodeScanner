...
import com.websarva.wings.android.jan_scanner.databinding.ActivityBarcodeScannerBinding as ActivityBinding
...
class BarcodeScannerActivity : AppCompatActivity() {
    ...
    private fun vibrate() {
        ...
        if (!settings.vibrate) return
        if (VERSION.SDK_INT >= VERSION_CODES.O /* O = Android Oreo API26*/) {
            vibrator.vibrate(
                VibrationEffect.createOneShot(30, VibrationEffect.DEFAULT_AMPLITUDE),
            )
        } else {
